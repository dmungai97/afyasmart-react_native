...
//
// This exists instead of the `firebase-admin` npm package because Admin
// SDK's Firestore client depends on @grpc/grpc-js / Node http2 internals
// that don't reliably run in the Deno-based Edge Functions runtime. Talking
// to the plain HTTPS REST API has no native dependencies and works anywhere.
//
// A service-account-authenticated request here goes through the same
// privileged Firestore API path as the Admin SDK, so — like the Admin SDK —
// it bypasses firestore.rules entirely (rules only gate the mobile/web
// client protocol). See firestore.rules for the collections this touches.
...
// exchanges it for a short-lived OAuth2 access token scoped to Firestore.
// Cached in-memory so a warm isolate reuses it instead of re-signing/
...
async function getAccessToken(): Promise<string> {
  ...
  // Secrets are single-line env vars, so a real PEM's newlines arrive escaped.
...
// path is a document path relative to the database root, e.g. "users/uid".
// deno-lint-ignore no-explicit-any
...
// Merge-patch, mirroring the Admin SDK's `.set(data, { merge: true })`.
// deno-lint-ignore no-explicit-any
...
// collectionPath is a top-level (or odd-segment) collection path, e.g.
// "paymentRequests". Returns the new document's auto-generated id.
// deno-lint-ignore no-explicit-any
...
// `FieldValue.increment(...)` — a plain read-modify-write PATCH would race
// two concurrent commissions against the same balance.
