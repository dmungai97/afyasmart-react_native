...
export default function DoctorsScreen() {
  ...
  // coords kept in BOTH state (for UI reactivity) and ref (for callbacks)
  const [coords,          setCoords]         = useState<{ lat: number; lng: number } | null>(null);
  const coordsRef = useRef<{ lat: number; lng: number } | null>(null); // ← fix: always-current ref
  ...
  // guard: skip filter effect on the very first render
  const hasMounted = useRef(false); // ← fix: mount guard
  ...
  const loadDoctors = useCallback(async (
    ...
    setLoading(true);
    try {
      // ← fix: use coordsRef.current instead of coords state
      ...
      const data = await fetchNearbyDoctors(token!, {
        ...
        // Only pass coords when the user explicitly requested proximity filtering.
        // Sending lat/lng without radius causes the backend to apply a default
        // distance cutoff, returning 0 results for users far from seeded data.
  ...
  }, [token, activeRegion, activeSpec, nearMeOnly]); // ← fix: coords removed from deps
  ...
  useEffect(() => {
    (async () => {
      ...
      // Now loadDoctors reads from coordsRef.current — always accurate
      await loadDoctors(undefined, resolvedCoords);
    })();
  }, []); // eslint-disable-line react-hooks/exhaustive-deps


  // ── Filter change effect — skips mount, fires only on filter changes ──────


  useEffect(() => {
    // ← fix: skip the boot render
    if (!hasMounted.current) {
      hasMounted.current = true;
      return;
    }
    loadDoctors();
  }, [activeRegion, activeSpec, nearMeOnly, loadDoctors]); // ← fix: loadDoctors in deps
  ...
  return (
    <View style={styles.container}>
      ...
      {/* ── Near Me toggle — only visible when GPS is available ── */}
      {coords ? (
